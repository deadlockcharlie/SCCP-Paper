...
A second observation is that the Cartesian product of commutative monoids is also a commutative monoid.
Thus, for any object $T$ with state $S$ and mutators $\Delta_m$ if $(S,\Delta_m)$ can be split into a set $(S,\Delta_1), \ldots, (S,\Delta_m)$ such that each $(S,\Delta_i)$ is liftable in the sense of \reftheo{lift}, then $T$ admits a CRDT implemenation.
%
This happens with PN-counter (\refsec{background});
in this case $m=2$, $\Delta_1$ encodes increments, and $\Delta_2$ encodes decrements.
%
Going back to the CRDT sequence, removal is implemented by taking the Cartesian product of $(\mathcal{P}, \cup)$ together a commutative monoid $(D, \land)$ where $D$ is a map from the sequence domain to $\{0,1\}$ and $(D \land D')[x]$ equals $0$ if either $D[x]=0$ or $D'[x]=0$ and $1$ otherwise.
