...
That is, for each read operation $r$, $\resp(r)$ results from some linearization $l_{r}$ of the updates visible to $r$ consistent with $\hb$;
...
\item (Eventual visibility)
  An operation will eventually be visible to another operation invoked after the completion of op.
...
\begin{definition}[CRDT]
    A conflict-free replicated datatype, denoted $\mathbb{T} \coloneq (\mathbb{S}, \bot, u, \sqcup)$, is a tuple consisting of a set of states $\mathbb{S}$, containing an initial state $\bot \in \mathbb{S}$. 
